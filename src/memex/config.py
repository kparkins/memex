...
class EmbeddingSettings(BaseSettings):
    ...
    Args:
        model: litellm model identifier, e.g. ``"text-embedding-3-small"``
            for OpenAI hosted, ``"openai/<model>"`` for any
            OpenAI-compatible local server (LM Studio, mlx-omni-server,
            vLLM, text-embeddings-inference), or ``"ollama/<model>"``
            for a native Ollama backend.

"""Integration tests for profile/shadow/judgment/calibration storage in Neo4jStore.

Covers all MemoryStore methods added in Phases 1-4 against a live Neo4j
instance.  All tests are skipped when Neo4j is not reachable, mirroring
the skip convention used by ``test_neo4j_store.py`` and conftest.py.
"""
...
# -- Labels cleared between tests for isolation ------------------------------

_PROFILE_LABELS = (
    NodeLabel.RETRIEVAL_PROFILE,
    NodeLabel.SHADOW_RETRIEVAL_PROFILE,
    NodeLabel.QUERY_JUDGMENT,
    NodeLabel.CALIBRATION_AUDIT_REPORT,
)
...
async def store(neo4j_driver: AsyncDriver) -> Neo4jStore:
    """Provide a Neo4jStore backed by the shared test driver."""
    await ensure_schema(neo4j_driver)
    return Neo4jStore(neo4j_driver)


@pytest.fixture(autouse=True)
async def _clean_profile_nodes(neo4j_driver: AsyncDriver) -> None:
    """Delete all profiling-layer nodes before each test."""
    async with neo4j_driver.session() as session:
        for label in _PROFILE_LABELS:
            await (
                await session.run(
                    f"MATCH (n:{label.value}) DETACH DELETE n"
                )
            ).consume()
...
async def test_profile_roundtrip(store: Neo4jStore) -> None:
    ...
    got = await store.get_retrieval_profile("proj-1")
...
async def test_profile_replace_atomic(store: Neo4jStore) -> None:
    """Saving generation=2 replaces generation=1; get returns generation=2."""
    await store.save_retrieval_profile(_make_profile("proj-2", generation=1))
    await store.save_retrieval_profile(_make_profile("proj-2", generation=2))

    got = await store.get_retrieval_profile("proj-2")
...
async def test_shadow_independent_of_active(store: Neo4jStore) -> None:
    ...
    await store.save_retrieval_profile(active)
    await store.save_shadow_profile(shadow)

    got_active = await store.get_retrieval_profile("proj-3")
    got_shadow = await store.get_shadow_profile("proj-3")

    assert got_active is not None and got_active.k_lex == 1.0
    assert got_shadow is not None and got_shadow.k_lex == 9.9


async def test_clear_shadow(store: Neo4jStore) -> None:
    ...
    await store.clear_shadow_profile("proj-4")

    assert await store.get_shadow_profile("proj-4") is None
    assert await store.get_retrieval_profile("proj-4") is not None


async def test_clear_shadow_idempotent(store: Neo4jStore) -> None:
    """Clearing a non-existent shadow profile raises no error."""
    await store.clear_shadow_profile("proj-5")
    await store.clear_shadow_profile("proj-5")
...
async def test_rollback_requires_previous(store: Neo4jStore) -> None:
    ...
    result = await store.rollback_retrieval_profile("proj-6")

    assert result is None
    got = await store.get_retrieval_profile("proj-6")
    assert got is not None and got.generation == 1


async def test_rollback_single_level(store: Neo4jStore) -> None:
    ...
    await store.save_retrieval_profile(v2)

    reverted = await store.rollback_retrieval_profile("proj-7")
    ...
    active = await store.get_retrieval_profile("proj-7")
...
async def test_judgment_save_get(store: Neo4jStore) -> None:
    """save_judgment then get_recent_judgments returns the saved judgment."""
    j = _make_judgment("proj-8")
    await store.save_judgment(j)

    results = await store.get_recent_judgments("proj-8")
...
async def test_judgment_get_recent_ordering(store: Neo4jStore) -> None:
    ...
    results = await store.get_recent_judgments("proj-9")

    assert [r.id for r in results] == [j3.id, j2.id, j1.id]


async def test_judgment_get_recent_since_filter(store: Neo4jStore) -> None:
    ...
    await store.save_judgment(before)
    await store.save_judgment(after)

    cutoff = datetime(2026, 4, 22, 10, tzinfo=UTC)
    results = await store.get_recent_judgments("proj-10", since=cutoff)

    assert len(results) == 1
    assert results[0].id == after.id


async def test_judgment_get_recent_limit(store: Neo4jStore) -> None:
    ...
    results = await store.get_recent_judgments("proj-11", limit=3)

    assert len(results) == 3


async def test_judgment_replace_by_id(store: Neo4jStore) -> None:
    ...
    await store.save_judgment(labeled)

    results = await store.get_recent_judgments("proj-12")
...
async def test_get_labeled_judgments_filters_and_orders(
    ...
    results = await store.get_labeled_judgments("proj-12b")
...
async def test_calibration_report_save_get(store: Neo4jStore) -> None:
    """save_calibration_report then get_calibration_report returns its fields."""
    report = _make_calibration_report("proj-13")
    await store.save_calibration_report(report)

    got = await store.get_calibration_report(report.report_id)
...
async def test_calibration_report_list_newest_first(
    ...
    for r in (r1, r2, r3):
        await store.save_calibration_report(r)

    results = await store.list_calibration_reports("proj-14")
...
async def test_calibration_report_scoped_to_project(
    ...
    await store.save_calibration_report(r_p1)
    await store.save_calibration_report(r_p2)

    results = await store.list_calibration_reports("proj-15-p1")
